...
<div>
      ...
      <div v-if="currentItem == items[0]">
        ...
        <v-col  v-for="event in this.$store.getters['sortBar/getCurrentUserEventItems'].createdEvents" :key="event.eventId" cols = "4">
            <v-card
            class="mx-auto my-12"
            max-width="374"
            >
    ...
    <div v-else>
      <v-row class = "d-flex align-start justify-start  flex-row">
      <v-col  v-for="event in this.$store.getters['sortBar/getCurrentUserEventItems'].participantEvents" :key="event.eventId" cols = "4">
          <v-card
          class="mx-auto my-12"
          max-width="374"
          >
...
export default {
  ...
  computed: {
      ...
      UnsubscribeClicked(){
          return event => { 
            ...
            this.$store.commit('myEvents/removeUserAsParticipant', event);
      ...
      UpdateClicked(){
        return event => {
          this.$router.push({ name: 'usercreatedeventpage', params: {event}})
        };
      },
      DetailsClicked(){
        return event => {
          this.$router.push({ name: 'userparticipanteventpage', params: {event}})
        };
      },
      CancelClicked(){
        return event => {
          this.$store.dispatch("myEvents/cancelEvent", event);
        }
      }
  },
  methods: {
  ...
  data: function() {
    return {
      items:[
        'User created active events',
        'Active events where user participate'
      ],
      currentItem: null
    }
  },
